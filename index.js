...
app.get("/health", (req, res) => {
  res.status(200).json({
    ...
    role_id: process.env.ROLE_ID || "not set",
...
function requiredEnvCheck() {
  ...
  if (!process.env.CHANNEL_ID) missing.push("CHANNEL_ID");
  if (!process.env.ROLE_ID) missing.push("ROLE_ID");
...
// 只抽指定身分組的人
async function getRandomMemberByRole(guild) {
  ...
  if (!role) {
    console.log(`找不到身分組 ROLE_ID：${roleId}`);
    return null;
  }
  ...
  if (memberArray.length === 0) {
    console.log(`身分組「${role.name}」裡面沒有可抽選的非 Bot 成員`);
    return null;
  }
...
// 發送情話
async function sendLoveMessage(targetChannel = null) {
  try {
    const channel = targetChannel || await client.channels.fetch(process.env.CHANNEL_ID);
    ...
    if (!channel.guild) {
      console.log("這不是伺服器文字頻道，無法抽身分組成員");
      return;
    }

    const randomMember = await getRandomMemberByRole(channel.guild);

    if (!randomMember) {
      await channel.send("今天找不到有指定身分組的人 QQ");
      return;
    }
...
// 每天 09:00～20:59 之間隨機一個時間發送
function startRandomDailySchedule() {
  let todayRandomHour = Math.floor(Math.random() * 12) + 9;
  let todayRandomMinute = Math.floor(Math.random() * 60);
  let scheduledDate = getTaiwanDateString();
  ...
  cron.schedule(
    ...
    async () => {
      ...
      // 換日後重新抽今天的隨機時間
      if (scheduledDate !== taiwanDate) {
        scheduledDate = taiwanDate;
        lastSentDate = null;
        todayRandomHour = Math.floor(Math.random() * 12) + 9;
        todayRandomMinute = Math.floor(Math.random() * 60);

        console.log(
          `新的一天，重新抽發送時間：${todayRandomHour}:${String(todayRandomMinute).padStart(2, "0")}`
        );
      }
...
client.once("ready", () => {
  ...
  if (!requiredEnvCheck()) {
    console.log("環境變數缺少，Bot 已登入但不會正常發送情話。");
    return;
  }

  console.log(`指定頻道 CHANNEL_ID：${process.env.CHANNEL_ID}`);
  console.log(`抽選身分組 ROLE_ID：${process.env.ROLE_ID}`);
...
client.on("messageCreate", async message => {
  ...
  if (message.content === "!情話") {
    await sendLoveMessage(message.channel);
  }
});
